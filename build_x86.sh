...
if [ "${SYSTEM}" = "Linux" ]; then
  ...
  export TOOLCHAIN=$NDK/toolchains/x86-4.9/prebuilt/linux-x86_64
fi
...
export PLATFORM=$NDK/platforms/${ANDROID_API}/arch-x86
...
# 编译x264
if [ ! -x "libx264/build_x264_x86.sh" ]; then
    echo "can not find x264 build script"
else
    chmod a+x ./libx264/build_x264_x86.sh
    ./libx264/build_x264_x86.sh
fi

# 编译fdk-aac
if [ ! -x "fdk-aac/build_fdk-aac_x86.sh" ]; then
    echo "can not find x264 build script"
else
    chmod a+x ./fdk-aac/build_fdk-aac_x86.sh
    ./fdk-aac/build_fdk-aac_x86.sh
fi

# 编译ffmpeg
if [ ! -x "ffmpeg/build_ffmpeg_x86.sh" ]; then
  echo "can not find ffmpeg build script"
else
    chmod a+x ./ffmpeg/build_ffmpeg_x86.sh
    ./ffmpeg/build_ffmpeg_x86.sh
fi
